#!/bin/bash
# Deployment script for Image Hotspot XBlock to Tutor dev
...
TEMP_DIR=$(mktemp -d)
mkdir -p "$TEMP_DIR/image-hotspot-xblock"

# Copy only what's needed (exclude frontend/ directory entirely)
cp "$XBLOCK_DIR/setup.py" "$TEMP_DIR/image-hotspot-xblock/"
[ -f "$XBLOCK_DIR/MANIFEST.in" ] && cp "$XBLOCK_DIR/MANIFEST.in" "$TEMP_DIR/image-hotspot-xblock/"
cp -r "$XBLOCK_DIR/$XBLOCK_NAME" "$TEMP_DIR/image-hotspot-xblock/"

# Remove frontend source directory (we only need public/ with built bundles)
rm -rf "$TEMP_DIR/image-hotspot-xblock/$XBLOCK_NAME/frontend"

echo "   ✓ Created clean deployment package"
echo "   ✓ Included: setup.py, $XBLOCK_NAME/*.py, $XBLOCK_NAME/public/*.js"
echo ""

# Step 3 - Copy to containers
echo "📤 Step 3: Copying to containers..."
docker cp "$TEMP_DIR/image-hotspot-xblock" tutor_dev-cms-1:/tmp/
docker cp "$TEMP_DIR/image-hotspot-xblock" tutor_dev-lms-1:/tmp/
rm -rf "$TEMP_DIR"
echo "   ✓ Copied to both CMS and LMS containers"
echo ""

# Step 4 - Install in containers
echo "🔧 Step 4: Installing XBlock in containers..."
docker exec tutor_dev-cms-1 bash -c "cd /tmp/image-hotspot-xblock && pip install -e . --force-reinstall --no-deps"
docker exec tutor_dev-lms-1 bash -c "cd /tmp/image-hotspot-xblock && pip install -e . --force-reinstall --no-deps"
...
echo "   4. Add the XBlock to a unit: Advanced → Image Hotspot"
echo ""
echo "📝 To check logs:"
...
echo ""
echo "📖 Hotspot JSON Example:"
echo '   [{"id":"h1","label":"Region","shape":"rectangle","coordinates":[20,30,40,50],"correct":true,"points":1,"feedback":"Correct!"}]'
echo ""
