...
TEMP_DIR=$(mktemp -d)
mkdir -p "$TEMP_DIR/accordion"

# Copy only what's needed (exclude frontend/ directory entirely)
cp "$XBLOCK_DIR/setup.py" "$TEMP_DIR/accordion/"
[ -f "$XBLOCK_DIR/MANIFEST.in" ] && cp "$XBLOCK_DIR/MANIFEST.in" "$TEMP_DIR/accordion/"
cp -r "$XBLOCK_DIR/$XBLOCK_NAME" "$TEMP_DIR/accordion/"

# Remove frontend source directory (we only need public/ with built bundles)
rm -rf "$TEMP_DIR/accordion/$XBLOCK_NAME/frontend"
...
docker cp "$TEMP_DIR/accordion" tutor_dev-cms-1:/tmp/
docker cp "$TEMP_DIR/accordion" tutor_dev-lms-1:/tmp/
rm -rf "$TEMP_DIR"
...
docker exec tutor_dev-cms-1 bash -c "cd /tmp/accordion && pip install -e . --force-reinstall --no-deps"
docker exec tutor_dev-lms-1 bash -c "cd /tmp/accordion && pip install -e . --force-reinstall --no-deps"
