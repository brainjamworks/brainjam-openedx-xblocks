#!/bin/bash
# ARCHITECTURAL: Deployment script for AudioPlayer to Tutor dev
#
...
XBLOCK_DIR="/Users/brainjam/brainjam-openedx-xblocks/xblocks/audio-player-xblock"
XBLOCK_NAME="audio_player"

echo "🚀 Deploying AudioPlayer to Tutor dev..."
echo ""
...
TEMP_DIR=$(mktemp -d)
mkdir -p "$TEMP_DIR/audio-player-xblock"

# Copy only what's needed (exclude frontend/ directory entirely)
cp "$XBLOCK_DIR/setup.py" "$TEMP_DIR/audio-player-xblock/"
[ -f "$XBLOCK_DIR/MANIFEST.in" ] && cp "$XBLOCK_DIR/MANIFEST.in" "$TEMP_DIR/audio-player-xblock/"
cp -r "$XBLOCK_DIR/$XBLOCK_NAME" "$TEMP_DIR/audio-player-xblock/"

# Remove frontend source directory (we only need public/ with built bundles)
rm -rf "$TEMP_DIR/audio-player-xblock/$XBLOCK_NAME/frontend"
...
docker cp "$TEMP_DIR/audio-player-xblock" tutor_dev-cms-1:/tmp/
docker cp "$TEMP_DIR/audio-player-xblock" tutor_dev-lms-1:/tmp/
rm -rf "$TEMP_DIR"
...
docker exec tutor_dev-cms-1 bash -c "cd /tmp/audio-player-xblock && pip install -e . --force-reinstall --no-deps"
docker exec tutor_dev-lms-1 bash -c "cd /tmp/audio-player-xblock && pip install -e . --force-reinstall --no-deps"
...
echo "   4. Add the XBlock to a unit: Advanced → AudioPlayer"
echo ""
echo "📝 To check logs:"
