...
|-------------|---------------------|------------|
| `AUDIO_PLAYER` | `MY_WIDGET` | Python constants, JS function names |
| `audio_player` | `my_widget` | Python module names, file names |
| `AudioPlayer` | `MyWidget` | Python class names, React component names |
| `{xblock-name}` | `my-widget` | Package names, URLs, display names |

**Example:**
- If you're building a "Quiz Timer" XBlock:
  - `AUDIO_PLAYER` → `QUIZ_TIMER`
  - `audio_player` → `quiz_timer`
  - `AudioPlayer` → `QuizTimer`
  - `{xblock-name}` → `quiz-timer`

### 3. Rename Directories

```bash
mv audio_player quiz_timer  # Use your actual XBlock name
```
...
├── audio_player/                 # Python package (RENAME THIS)
│   ├── __init__.py               # Package init
│   ├── audio_player.py          # Main XBlock class (RENAME THIS)
...
1. **Bootstrap loaders** (`audio_player/static/*.js`)
   - jQuery unwrapping pattern
   - Error handling structure
   - *Only change function names during find-replace*

2. **CSS loading** (`audio_player/audio_player.py`)
...
2. **Python XBlock Class** (`audio_player/audio_player.py`)
   - Fields and handlers
   - Business logic
   - Data validation
...
2. Edit Python logic in `audio_player/audio_player.py`
...
4. CSS files exist in `audio_player/public/`
...
3. Check logs: `docker logs tutor_dev-cms-1 | grep audio_player`
...
- [ ] Replaced all placeholders (`AUDIO_PLAYER`, `audio_player`, etc.)
- [ ] Renamed `audio_player` directory
- [ ] Renamed `audio_player.py` file
