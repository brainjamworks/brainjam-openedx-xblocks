...
|-------------|---------------------|------------|
| `IMAGE_COMMENTARY` | `MY_WIDGET` | Python constants, JS function names |
| `image_commentary` | `my_widget` | Python module names, file names |
| `ImageCommentary` | `MyWidget` | Python class names, React component names |
| `image-commentary` | `my-widget` | Package names, URLs, display names |

**Example:**
- If you're building a "Quiz Timer" XBlock:
  - `IMAGE_COMMENTARY` → `QUIZ_TIMER`
  - `image_commentary` → `quiz_timer`
  - `ImageCommentary` → `QuizTimer`
  - `image-commentary` → `quiz-timer`

### 3. Rename Directories

```bash
mv image_commentary quiz_timer  # Use your actual XBlock name
```
...
├── image_commentary/                 # Python package (RENAME THIS)
│   ├── __init__.py               # Package init
│   ├── image_commentary.py          # Main XBlock class (RENAME THIS)
...
1. **Bootstrap loaders** (`image_commentary/static/*.js`)
   - jQuery unwrapping pattern
   - Error handling structure
   - *Only change function names during find-replace*

2. **CSS loading** (`image_commentary/image_commentary.py`)
...
2. **Python XBlock Class** (`image_commentary/image_commentary.py`)
   - Fields and handlers
   - Business logic
   - Data validation
...
2. Edit Python logic in `image_commentary/image_commentary.py`
...
4. CSS files exist in `image_commentary/public/`
...
3. Check logs: `docker logs tutor_dev-cms-1 | grep image_commentary`
...
- [ ] Replaced all placeholders (`IMAGE_COMMENTARY`, `image_commentary`, etc.)
- [ ] Renamed `image_commentary` directory
- [ ] Renamed `image_commentary.py` file
