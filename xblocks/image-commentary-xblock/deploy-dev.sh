#!/bin/bash
# ARCHITECTURAL: Deployment script for ImageCommentary to Tutor dev
#
...
TEMP_DIR=$(mktemp -d)
mkdir -p "$TEMP_DIR/image-commentary"

# Copy only what's needed (exclude frontend/ directory entirely)
cp "$XBLOCK_DIR/setup.py" "$TEMP_DIR/image-commentary/"
[ -f "$XBLOCK_DIR/MANIFEST.in" ] && cp "$XBLOCK_DIR/MANIFEST.in" "$TEMP_DIR/image-commentary/"
cp -r "$XBLOCK_DIR/$XBLOCK_NAME" "$TEMP_DIR/image-commentary/"

# Remove frontend source directory (we only need public/ with built bundles)
rm -rf "$TEMP_DIR/image-commentary/$XBLOCK_NAME/frontend"
...
docker cp "$TEMP_DIR/image-commentary" tutor_dev-cms-1:/tmp/
docker cp "$TEMP_DIR/image-commentary" tutor_dev-lms-1:/tmp/
rm -rf "$TEMP_DIR"
...
docker exec tutor_dev-cms-1 bash -c "cd /tmp/image-commentary && pip install -e . --force-reinstall --no-deps"
docker exec tutor_dev-lms-1 bash -c "cd /tmp/image-commentary && pip install -e . --force-reinstall --no-deps"
...
echo "   4. Add the XBlock to a unit: Advanced → ImageCommentary"
echo ""
echo "📝 To check logs:"
