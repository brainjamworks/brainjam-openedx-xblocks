#!/bin/bash
# ARCHITECTURAL: Deployment script for Accordion to Tutor dev
#
...
XBLOCK_DIR="/Users/brainjam/brainjam-openedx-xblocks/xblocks/content-blocks"
XBLOCK_NAME="content_blocks"

echo "🚀 Deploying Content Blocks to Tutor dev..."
echo ""
...
TEMP_DIR=$(mktemp -d)
mkdir -p "$TEMP_DIR/content_blocks"

# Copy only what's needed (exclude frontend/ directory entirely)
cp "$XBLOCK_DIR/setup.py" "$TEMP_DIR/content_blocks/"
[ -f "$XBLOCK_DIR/MANIFEST.in" ] && cp "$XBLOCK_DIR/MANIFEST.in" "$TEMP_DIR/content_blocks/"
cp -r "$XBLOCK_DIR/$XBLOCK_NAME" "$TEMP_DIR/content_blocks/"

# Remove frontend source directory (we only need public/ with built bundles)
rm -rf "$TEMP_DIR/content_blocks/$XBLOCK_NAME/frontend"
...
docker cp "$TEMP_DIR/content_blocks" tutor_dev-cms-1:/tmp/
docker cp "$TEMP_DIR/content_blocks" tutor_dev-lms-1:/tmp/
rm -rf "$TEMP_DIR"
...
docker exec tutor_dev-cms-1 bash -c "cd /tmp/content_blocks && pip install -e . --force-reinstall --no-deps"
docker exec tutor_dev-lms-1 bash -c "cd /tmp/content_blocks && pip install -e . --force-reinstall --no-deps"
...
echo "   4. Add the XBlock to a unit: Advanced → Accordion"
echo ""
echo "📝 To check logs:"
