#!/bin/bash
# ARCHITECTURAL: Deployment script for {XBlockName} to Tutor dev
#
...
XBLOCK_DIR="/Users/brainjam/liverpool-dental-learning-hub/brainjam-openedx-xblocks/{xblock-name}"
XBLOCK_NAME="{xblock_name}"

echo "🚀 Deploying {XBlockName} to Tutor dev..."
echo ""
...
TEMP_DIR=$(mktemp -d)
mkdir -p "$TEMP_DIR/{xblock-name}"

# Copy only what's needed (exclude frontend/ directory entirely)
cp "$XBLOCK_DIR/setup.py" "$TEMP_DIR/{xblock-name}/"
[ -f "$XBLOCK_DIR/MANIFEST.in" ] && cp "$XBLOCK_DIR/MANIFEST.in" "$TEMP_DIR/{xblock-name}/"
cp -r "$XBLOCK_DIR/$XBLOCK_NAME" "$TEMP_DIR/{xblock-name}/"

# Remove frontend source directory (we only need public/ with built bundles)
rm -rf "$TEMP_DIR/{xblock-name}/$XBLOCK_NAME/frontend"
...
docker cp "$TEMP_DIR/{xblock-name}" tutor_dev-cms-1:/tmp/
docker cp "$TEMP_DIR/{xblock-name}" tutor_dev-lms-1:/tmp/
rm -rf "$TEMP_DIR"
...
docker exec tutor_dev-cms-1 bash -c "cd /tmp/{xblock-name} && pip install -e . --force-reinstall --no-deps"
docker exec tutor_dev-lms-1 bash -c "cd /tmp/{xblock-name} && pip install -e . --force-reinstall --no-deps"
...
echo "   4. Add the XBlock to a unit: Advanced → {XBlockName}"
echo ""
echo "📝 To check logs:"
