...
|-------------|---------|------------|
| `{XBLOCK_NAME}` | `QUIZ_TIMER` | Python constants, JS function names |
| `{xblock_name}` | `quiz_timer` | Python module names, file names |
| `{XBlockName}` | `QuizTimer` | Python class names, React components |
...
Edit `{xblock_name}/{xblock_name}.py`:
...
│   ├── __init__.py
│   ├── {xblock_name}.py          # ✅ ScorableXBlockMixin included
...
See commented examples in `{xblock_name}.py` and `types.ts` for complete multi-part implementation patterns.
