...
INSTALL_PATH=$(docker exec tutor_dev-cms-1 pip show image-annotation-xblock 2>/dev/null | grep "Location:" | awk '{print $2}')
...
sync_files() {
    ...
    if [ -f "$PUBLIC_DIR/studio-ui.js" ]; then
        docker cp "$PUBLIC_DIR/studio-ui.js" "tutor_dev-cms-1:$INSTALL_PATH/image-annotation-xblock/$XBLOCK_NAME/public/"
    fi
    if [ -f "$PUBLIC_DIR/studio-ui.css" ]; then
        docker cp "$PUBLIC_DIR/studio-ui.css" "tutor_dev-cms-1:$INSTALL_PATH/image-annotation-xblock/$XBLOCK_NAME/public/"
    fi

    # Sync to LMS (Student view)
    if [ -f "$PUBLIC_DIR/student-ui.js" ]; then
        docker cp "$PUBLIC_DIR/student-ui.js" "tutor_dev-lms-1:$INSTALL_PATH/image-annotation-xblock/$XBLOCK_NAME/public/"
    fi
    if [ -f "$PUBLIC_DIR/student-ui.css" ]; then
        docker cp "$PUBLIC_DIR/student-ui.css" "tutor_dev-lms-1:$INSTALL_PATH/image-annotation-xblock/$XBLOCK_NAME/public/"
    fi
