...
✅ **CSS loading** in both student_view and studio_view
✅ **`process.env.NODE_ENV` definition** in Vite config
✅ **React 17 + Paragon 22.x** version locking
✅ **IntlProvider wrapper** - required for Form/Alert components
✅ **Save notification pattern** - proper Studio modal management
...
| `timeline_presentation` | `my_widget` | Python module names, file names |
| `TimelinePresentation` | `MyWidget` | Python class names, React component names |
| `timeline-presentation-xblock` | `my-widget` | Package names, URLs, display names |

**Example:**
- If you're building a "Quiz Timer" XBlock:
  - `{XBLOCK_NAME}` → `QUIZ_TIMER`
  - `timeline_presentation` → `quiz_timer`
  - `TimelinePresentation` → `QuizTimer`
  - `timeline-presentation-xblock` → `quiz-timer`

### 3. Rename Directories

```bash
mv timeline_presentation quiz_timer  # Use your actual XBlock name
```
...
├── timeline_presentation/                 # Python package (RENAME THIS)
│   ├── __init__.py               # Package init
│   ├── timeline_presentation.py          # Main XBlock class (RENAME THIS)
...
1. **Bootstrap loaders** (`timeline_presentation/static/*.js`)
   - jQuery unwrapping pattern
   - Error handling structure
   - *Only change function names during find-replace*

2. **CSS loading** (`timeline_presentation/timeline_presentation.py`)
   - `frag.add_css_url()` in both student_view and studio_view
   - Critical for loading Paragon styles and custom styling
   - Without this, no styles will load!
...
4. **API helper** (`frontend/src/common/api.ts`)
   - CSRF token handling
   - Standard error handling
   - *Use as-is or extend*

5. **Package versions** (`frontend/package.json`)
   - React 17.0.2 (required for OpenEdX compatibility)
   - Paragon 22.5.1 (matches OpenEdX)
   - react-intl (provided by Paragon, required by Form/Alert components)
   - *Don't upgrade these*

6. **Entry points** (`frontend/src/*/index.tsx`)
   - React render with IntlProvider wrapper (required for Form/Alert components)
...
7. **Save notification pattern** (`frontend/src/studio-ui/StudioView.tsx`)
   - Call `runtime.notify('save', { state: 'start' })` before save
   - Call `runtime.notify('save', { state: 'end' })` after save (success or error)
   - Ensures Studio modal management works correctly
   - *Don't remove these notifications*
...
2. **Python XBlock Class** (`timeline_presentation/timeline_presentation.py`)
   - Fields and handlers
   - Business logic
   - Data validation
...
2. Edit Python logic in `timeline_presentation/timeline_presentation.py`
...
### SCSS Imports (Different Path!)

**IMPORTANT:** SCSS imports use `/src/` not `/dist/`:

```scss
// ✅ CORRECT - SCSS imports from /src/
...
// ❌ WRONG - Don't use /dist/ for SCSS
@import '~@openedx/paragon/dist/Button';
```

**Why different paths?**
- JavaScript: `/dist/` = Compiled JS bundles
- SCSS: `/src/` = Source SCSS files

### Which Components Need IntlProvider?

Based on testing with Paragon 22.5.1:

**✅ Work WITHOUT IntlProvider:**
- Icon
- Card
- Button (standalone)
- Collapsible

**❌ REQUIRE IntlProvider:**
- Form (Form.Group, Form.Control, Form.Label, etc.)
- Alert
- Modal (with built-in messaging)

**Why the template includes IntlProvider:**
The template includes `<IntlProvider locale="en">` wrapper in both views because studio editing commonly uses Form and Alert components. This ensures all Paragon components work without errors.

**Note:** react-intl is provided by @openedx/paragon as a peer dependency, so it adds no extra bundle size.
...
### React Intl Error: "Could not find required `intl` object"

✅ **Fixed in template** - Both entry points include:
```typescript
...
This error occurs when using Form or Alert components without IntlProvider. The template includes IntlProvider by default.

### Save notification error: "Cannot read properties of undefined (reading 'hide')"

✅ **Fixed in template** - StudioView includes:
```typescript
runtime.notify('save', { state: 'start' });  // Before save
// ... save operation ...
runtime.notify('save', { state: 'end' });    // After save
```

This error occurs when calling 'end' without calling 'start' first. Always call both in sequence.

### Paragon styles not loading

✅ **Fixed in template** - Both views include:
```python
frag.add_css_url(self.runtime.local_resource_url(self, 'public/student-ui.css'))
```

If you removed this line or styles still aren't loading, check that:
1. CSS loading lines exist in both `student_view()` and `studio_view()`
2. `minimal-paragon.scss` is imported in your entry point (`index.tsx`)
3. Build completed successfully: `npm run build`
4. CSS files exist in `timeline_presentation/public/`
5. XBlock was redeployed after adding CSS loading: `./deploy-dev.sh`
...
3. Check logs: `docker logs tutor_dev-cms-1 | grep timeline_presentation`

## 📖 Further Reading

- **[Paragon Styling Guide](./PARAGON_STYLING_GUIDE.md)** - Comprehensive guide to styling XBlocks with Paragon
...
- [ ] Replaced all placeholders (`{XBLOCK_NAME}`, `timeline_presentation`, etc.)
- [ ] Renamed `timeline_presentation` directory
- [ ] Renamed `timeline_presentation.py` file
