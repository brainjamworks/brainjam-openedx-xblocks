#!/bin/bash
# ARCHITECTURAL: Deployment script for TimelinePresentation to Tutor dev
#
...
XBLOCK_DIR="/Users/brainjam/brainjam-openedx-xblocks/xblocks/timeline-presentation-xblock"
XBLOCK_NAME="timeline_presentation"

echo "🚀 Deploying TimelinePresentation to Tutor dev..."
echo ""
...
TEMP_DIR=$(mktemp -d)
mkdir -p "$TEMP_DIR/timeline-presentation-xblock"

# Copy only what's needed (exclude frontend/ directory entirely)
cp "$XBLOCK_DIR/setup.py" "$TEMP_DIR/timeline-presentation-xblock/"
[ -f "$XBLOCK_DIR/MANIFEST.in" ] && cp "$XBLOCK_DIR/MANIFEST.in" "$TEMP_DIR/timeline-presentation-xblock/"
cp -r "$XBLOCK_DIR/$XBLOCK_NAME" "$TEMP_DIR/timeline-presentation-xblock/"

# Remove frontend source directory (we only need public/ with built bundles)
rm -rf "$TEMP_DIR/timeline-presentation-xblock/$XBLOCK_NAME/frontend"
...
docker cp "$TEMP_DIR/timeline-presentation-xblock" tutor_dev-cms-1:/tmp/
docker cp "$TEMP_DIR/timeline-presentation-xblock" tutor_dev-lms-1:/tmp/
rm -rf "$TEMP_DIR"
...
docker exec tutor_dev-cms-1 bash -c "cd /tmp/timeline-presentation-xblock && pip install -e . --force-reinstall --no-deps"
docker exec tutor_dev-lms-1 bash -c "cd /tmp/timeline-presentation-xblock && pip install -e . --force-reinstall --no-deps"
...
echo "   4. Add the XBlock to a unit: Advanced → TimelinePresentation"
echo ""
echo "📝 To check logs:"
