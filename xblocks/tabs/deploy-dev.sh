#!/bin/bash
# ARCHITECTURAL: Deployment script for Tabs to Tutor dev
#
...
TEMP_DIR=$(mktemp -d)
mkdir -p "$TEMP_DIR/tabs"

# Copy only what's needed (exclude frontend/ directory entirely)
cp "$XBLOCK_DIR/setup.py" "$TEMP_DIR/tabs/"
[ -f "$XBLOCK_DIR/MANIFEST.in" ] && cp "$XBLOCK_DIR/MANIFEST.in" "$TEMP_DIR/tabs/"
cp -r "$XBLOCK_DIR/$XBLOCK_NAME" "$TEMP_DIR/tabs/"

# Remove frontend source directory (we only need public/ with built bundles)
rm -rf "$TEMP_DIR/tabs/$XBLOCK_NAME/frontend"
...
docker cp "$TEMP_DIR/tabs" tutor_dev-cms-1:/tmp/
docker cp "$TEMP_DIR/tabs" tutor_dev-lms-1:/tmp/
rm -rf "$TEMP_DIR"
...
docker exec tutor_dev-cms-1 bash -c "cd /tmp/tabs && pip install -e . --force-reinstall --no-deps"
docker exec tutor_dev-lms-1 bash -c "cd /tmp/tabs && pip install -e . --force-reinstall --no-deps"
...
echo "   4. Add the XBlock to a unit: Advanced → Tabs"
echo ""
echo "📝 To check logs:"
