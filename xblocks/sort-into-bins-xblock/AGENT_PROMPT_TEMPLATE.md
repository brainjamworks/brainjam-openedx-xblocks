...
|-------------|--------------|---------|
| `SortIntoBins` | PascalCase name | `TextCard` |
| `sort_into_bins` | snake_case name | `text_card` |
| `{xblock-name}` | kebab-case name | `text-card-xblock` |

**Files to update**:
- [ ] `sort_into_bins/sort_into_bins.py` → Rename file and update placeholders
- [ ] `sort_into_bins/static/student.js` → Update function names
- [ ] `sort_into_bins/static/studio.js` → Update function names
...
##### A. Python XBlock Class (`sort_into_bins/sort_into_bins.py`)
...
This builds both student-ui and studio-ui bundles to `sort_into_bins/public/`.
...
sort_into_bins/public/student-ui.css (if you have styles)
```
...
- [ ] All placeholders replaced (SortIntoBins, sort_into_bins, {xblock-name})
